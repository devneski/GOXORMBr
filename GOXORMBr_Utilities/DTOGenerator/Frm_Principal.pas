...
uses
  ...
  Mask, AnsiStrings, FireDAC.UI.Intf, FireDAC.VCLUI.Wait, FireDAC.Stan.ExprFuncs, FireDAC.Phys.SQLiteWrapper.Stat, FireDAC.Phys.SQLiteDef,
  FireDAC.Phys.MSSQLDef, FireDAC.Stan.Intf, FireDAC.Stan.Option, FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.DatS, FireDAC.Phys.Intf, FireDAC.DApt.Intf,
  FireDAC.Stan.Async, FireDAC.DApt, FireDAC.Stan.Def, FireDAC.Stan.Pool, FireDAC.Phys, FireDAC.Comp.Client,
  FireDAC.Comp.DataSet, FireDAC.Phys.MSSQL, FireDAC.Phys.SQLite, FireDAC.Comp.UI, System.Generics.Collections, System.StrUtils,
  Winapi.ShellAPI, FireDAC.Phys.ODBCBase,
...
type
  ...
  private
    ...
    function GetPKName(ATableName:String):String;
...
begin
  ...
  begin
    ...
    begin
      ...
      Conn.Params.Values['ODBCDriver']   := 'ODBC Driver 17 for SQL Server';
      Conn.Params.Values['ODBCAdvanced'] := 'Server='+Server+';Database='+Database;
    end
    else
    begin
...
begin
  ShellExecute(Application.Handle, PChar('open'),PChar('explorer.exe'), PChar(ExtractFilePath(ParamStr(0))+'Dtos'), nil, SW_NORMAL)
...
begin
  try
    ...
    Metadata.BaseObjectName  := lstTabelas.Items.Strings[index];
...
function TFrmPrincipal.GetPKName(ATableName:String): String;
begin
   with Metadata do
   begin
     Close;
     IndexFieldNames := '';
     BaseObjectName := ATableName;
     MetaInfoKind := mkPrimaryKeyFields;
     Open;
   end;
   Result := Metadata.FieldByName('COLUMN_NAME').AsString;
end;
...
//          NullableTipo := 'Nullable<' + NullableTipo + '>';
     ...
     //-------------------------------------------------------------------
     GetPK(_ListIndex);
     if (Metadata.Active = true) and (Metadata.RecordCount > 0) then
     begin
       if Metadata.Locate('COLUMN_NAME',_Campo,[loCaseInsensitive]) then
       _PrimaryKey.Add('  [PrimaryKey('+QuotedStr(_Campo)+', AutoInc, NoSort, False, '+QuotedStr('Chave primária')+')]');
     end;
     ...
     begin
       GetPK(_ListIndex);
       if Metadata.Locate('COLUMN_NAME',_Campo,[loCaseInsensitive]) then
       _Sequence.Add('  [Sequence('+QuotedStr('SEQ_'+_Campo)+')]');
     end;

     // Required
     if Entidade.FieldByName(_Campo).Required then
      _Propertys.Add('    [Restrictions([NoValidate, NotNull])]')
      else
      _Propertys.Add('    [Restrictions([NoValidate])]');
     ...
     _Propertys.Add('    [ColumnDBGrid(' + QuotedStr(_Campo) + ', ' + QuotedStr(_Campo) + ', ' + IntToStr(100) + ', ' + QuotedStr('Arial') + ', ' + IntToStr(10) + ', ' + 'clWindowText' + ', ' + 'False' + ', ' + _Align + ', ' + QuotedStr('PSQ;')+')]');

     _Propertys.Add('    [Dictionary(''' + _Campo + ''', ''Divergente do valor esperado.'', ' + _Default + ', '''', ' + _Mask + ', ' + _Align +')]');
...
begin
  ...
  begin
    ...
    if Entidade.FieldByName(Campo).DataType in [ftLargeint, ftSmallint, ftInteger, ftWord, ftAutoInc] then
       CreateBodyProperty(index,Campo,'Integer',ReadWrite,'taCenter','''0''',I)
    else
...
begin
  ...
  LoadPropertys(index);
  //
  memModel.Lines.Add('unit '+edtProjeto.Text+UpperCase(lstTabelas.Items.Strings[index] )+ ';');
  memModel.Lines.Add('');
  memModel.Lines.Add('interface');
  memModel.Lines.Add('');
  memModel.Lines.Add('uses');
  memModel.Lines.Add('  System.SysUtils, ');
  memModel.Lines.Add('  System.Classes, ');
  memModel.Lines.Add('  Vcl.Graphics, ');
  memModel.Lines.Add('  Data.DB, ');
  memModel.Lines.Add('  Generics.Collections, ');
  memModel.Lines.Add('');
  memModel.Lines.Add('  //goxormbr ');
  //Add Units Associadas
  for iKey  := 0 to _UsesRelations.Count -1 do
    memModel.Lines.Add(_UsesRelations[iKey] + ',');

  memModel.Lines.Add('  goxormbr.core.consts,');
  memModel.Lines.Add('  goxormbr.core.types.blob,');
  memModel.Lines.Add('  goxormbr.core.types.mapping,');
  memModel.Lines.Add('  goxormbr.core.mapping.classes,');
  memModel.Lines.Add('  goxormbr.core.mapping.register,');
  memModel.Lines.Add('  goxormbr.core.mapping.attributes,');
  memModel.Lines.Add('  //Application ');
  memModel.Lines.Add('  Core.Mapping.Attributes;');
  memModel.Lines.Add('');
  ...
  for iKey  := 0 to _Sequence.Count -1 do
  memModel.Lines.Add(_Sequence[iKey]);
  ...
  memModel.Lines.Add('  TDto'+lstTabelas.Items.Strings[index] + ' = class');
  ...
  if (_ConstructorDeclaration.Count > 0)or
     (_DestructorDeclaration.Count > 0) then
    memModel.Lines.Add('');
  //Add Constants name table
  memModel.Lines.Add('    const Table      = '+QuotedStr(lstTabelas.Items.Strings[index])+';');
  var LPK := GetPKName(lstTabelas.Items.Strings[index]);
  if LPK.Length > 0 then
  begin
    memModel.Lines.Add('    const PrimaryKey = '+QuotedStr(LPK)+';');
    memModel.Lines.Add('    const Sequence   = '+QuotedStr('SEQ_'+LPK)+';');
  end;
  memModel.Lines.Add('');
  ...
  memModel.Lines.Add('  TRegisterClass.RegisterEntity(TDto'+lstTabelas.Items.Strings[index]+')');
  memModel.Lines.Add('');
